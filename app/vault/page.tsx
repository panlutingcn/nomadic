  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-page)', display: 'flex', flexDirection: 'column' }}>
      <div style={{ flex: 1, padding: '14px 16px 10px' }}>
            ...
            <div style={{ maxHeight: editOpen ? 500 : 0, overflow: 'hidden', transition: 'max-height 300ms ease' }}>
              <div style={{ paddingTop: 14 }}>
                ...
                <div style={{ marginBottom: 14 }}>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>微信</div>
                  <div style={{ fontSize: 12, color: user?.user_metadata?.wechat_openid ? 'var(--text-primary)' : 'var(--text-muted)' }}>
                    {user?.user_metadata?.wechat_openid ? '已绑定' : '未绑定'}
                  </div>
                </div>
                ...
                <div style={{ marginTop: 16, paddingTop: 12, borderTop: '0.5px solid var(--border)' }}>
                  {!deleteConfirm ? (
                    ...
                    <div>
                      <div style={{ fontSize: 11, color: '#c04040', marginBottom: 8 }}>
                        此操作不可撤销，将永久删除你的账号与所有数据。
                      </div>
                      <input
                        type="text"
                        value={deleteInput}
        ...
        {/* Guest login bubble — shown below imprints list when not logged in */}
        {!user && (
          <>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              ...
              >
                ...
                <div style={{ fontSize: 13, fontWeight: 600, color: '#3d3020', marginBottom: 4 }}>登录账号 ✦</div>
                <div style={{ fontSize: 10, color: '#9a8a6a', marginBottom: 10 }}>保存你的城市与印迹</div>
                <div style={{ fontSize: 11, color: '#1D9E75', fontWeight: 500 }}>点击登录 →</div>
              </div>
            </div>
          </>
        )}
