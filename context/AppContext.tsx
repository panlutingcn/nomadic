...
const SAMPLE_USER_IMPRINTS: Imprint[] = [
  { id: 'sample-1', city: 'Berlin', author: '@nomad_kai', title: '施普雷河边的一个黄昏', narrative: '傍晚的施普雷河边，有人在废弃发电站的墙上作画，有人在河岸边开着露天派对。\n柏林的创业者大多不穿西装，但他们谈的事往往比任何办公室里的人更认真。\n这座城市有一种笃定感——不着急，但走得很远。\n理性与浪漫在这里共存，像两种矛盾的颜色调出了最对的底色。', tags: ['柏林', '德国', '创业氛围'], isPublic: true, likes: 7, createdAt: '2026.03.07', photo: 'https://images.unsplash.com/photo-1599946347371-68eb71b16afc?auto=format&fit=crop&w=600&q=80' },
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const likeImprint = (id: string) => {
    ...
    // Look up current likes in either user imprints or community imprints
    const current =
      imprints.find(i => i.id === id)?.likes ??
      communityImprints.find(i => i.id === id)?.likes ?? 0
    const newLikes = Math.max(0, current + delta)
    setImprints(prev => prev.map(i => i.id === id ? { ...i, likes: newLikes } : i))
    setCommunityImprints(prev => prev.map(i => i.id === id ? { ...i, likes: newLikes } : i))
